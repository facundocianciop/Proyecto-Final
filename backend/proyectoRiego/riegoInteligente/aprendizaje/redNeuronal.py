...
from redNeuronalUtils import *


modeloProduccion = None


def principal():
    global modeloProduccion
    # deteminamos la semilla 7
    numpy.random.seed(7)

    try:
        # cargamos el dataset
        dataset = numpy.loadtxt("riegoInteligente/aprendizaje/dataset.csv", delimiter=",")
...
def evaluarNuevaMedicion(mes, dia, cultivo, mecanismo, ppe, presionA, tempA, humA, radiacionS, tempS, humS, tsuelo=1):
    # global modeloProduccion

    if modeloProduccion is None:
        principal()

    eto = obtener_eto(mes)
    kc = obtener_kc(mes)
    eficienciaM = obtener_eficiencia_riego(mecanismo)
    ...
    nnr=etc-ppe
    nbr=nnr/eficienciaM
    consulta=numpy.array([mes,dia,cultivo,etc,eto,kc,p,wc,pm,tsuelo,mecanismo,eficienciaM,ppe,nnr,
                          nbr,tempA,humA,presionA,radiacionS,tempS,humS])

    resultado=modeloProduccion.predict(numpy.array([consulta, ])).round()
    print((numpy.array([consulta, ])).shape)
    print(modeloProduccion.predict(numpy.array([consulta, ])))
    print(resultado)
    if(resultado==0):
...
def llenarDatasetNuevosDatos():
    ...
    dataset = numpy.loadtxt("riegoInteligente/aprendizaje/valoresNuevos.csv", delimiter=",")
    print("hola")
    ...
    datosNuevos = open('riegoInteligente/aprendizaje/valoresNuevos.csv','w')
    datosNuevos.truncate()


def llenarDataset(datos):

    dataset=open('riegoInteligente/aprendizaje/dataset.csv','a')
...
def llenarNuevosDatos(consultaNueva):
    ...
    while (i < consultaNueva.__len__()):
        if (i == ((consultaNueva.__len__() - 1))):
            ...
            consultaGuardar = consultaGuardar + str(consultaNueva[i]) + ","
        i += 1
    dataset = open('riegoInteligente/aprendizaje/valoresNuevos.csv','a+')
...
# resultado = evaluarNuevaMedicion(12, 29, 1, 6.03, 1.05, 1, 1, 0.5, 0, 28, 27, 1007, 7086, 51, 58)
# print(resultado)
